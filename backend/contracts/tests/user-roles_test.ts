...
// Test case for ensuring a user has the default 'participant' role.
Clarinet.test({
    name: "User Roles: Ensures a user has the default 'participant' role",
    async fn(chain: Chain, accounts: Map<string, Account>) {
        // Get the deployer and wallet_1 accounts.
...
// Test case for allowing the contract owner (admin) to set a role.
Clarinet.test({
...
// Test case for preventing a non-admin from setting a role.
Clarinet.test({
    name: "User Roles: Prevents a non-admin from setting a role",
    async fn(chain: Chain, accounts: Map<string, Account>) {
        // Get the wallet_1 and wallet_2 accounts.
        const wallet1 = accounts.get('wallet_1')!;
...
// Test case for preventing setting an invalid role type.
Clarinet.test({
